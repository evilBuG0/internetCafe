...
public class JpaRepositoryImpl<T, ID extends Serializable> extends
		SimpleJpaRepository<T, ID> implements JpaRepository<T, ID> {
	...
	public int deleteInId(List<ID> ids) {
		...
		String ql = getQueryString(DELETE_ALL_QUERY_STRING,
	...
	public List<T> findAllById(List<ID> ids) {
		...
		String ql = getQueryString(READ_ALL_QUERY,
				entityInformation.getEntityName());
		logger.debug(ql);
		return applyAndBind(ql, ids, em).getResultList();
	}

	public Query applyAndBind(String queryString, List<ID> ids,
			EntityManager entityManager) {
