...
import com.ideal.oms.util.convert.ConversionException;
...
public class DateConverter extends AbstractConverter {
	...
	public Date parse(String value) {
		...
		if (value.matches("^[0-9]{4}$")) {
			format = new SimpleDateFormat(FMT_DATE_YEAR);
		} else if (value.matches("^[0-9]{4}-[0-9]{2}$")) {
			format = new SimpleDateFormat(FMT_DATE_MONTH);
		} else if (value.matches("^[0-9]{4}-[0-9]{2}-[0-9]{2}$")) {
			format = new SimpleDateFormat(FMT_DATE_DAY);
		} else if (value.matches("^[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}$")) {
			format = new SimpleDateFormat(FMT_DATE_TIME_HOUR);
		} else if (value
				.matches("^[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}$")) {
			format = new SimpleDateFormat(FMT_DATE_TIME_MINUTE);
		} else if (value
				.matches("^[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}$")) {
			format = new SimpleDateFormat(FMT_DATE_TIME_SECOND);
		} else if (value
				.matches("^[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2} [0-9]{3}$")) {
			format = new SimpleDateFormat(FMT_DATE_TIME_MILLISECOND);
		}
		if (format != null) {

			try {
				returnValue = format.parse(value);
